...
def get_search_page(request):
    ...
    if request.method == 'POST':
        first_name = request.POST.get('first_name')
        company = request.POST.get('company')
        items = AccessLog.objects.all().filter(Q(first_name__iexact=first_name) | Q(company__iexact=company) | Q(company__iexact=company))
        context = {
            'items': items
            }
        if items:
...
def edit_time_out_page(request, item_id):
    ...
    if request.method == 'POST' and 'time-out-submit-btn' in request.POST:
        ...
        messages.success(request, "Driver successfully checked out")
        return redirect('get_access_log')
    elif request.method == 'POST' and 'time-out-cancel-btn' in request.POST:
        
        messages.success(request, "Check out cancelled")
        return redirect('get_access_log')
